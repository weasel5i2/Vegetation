...
public class Canes {
	...
	public void growCanes(Block block) {
		...
		for (int i = 0; i < 150; i++) {
			plantBlock = blocks.getRandomTopBlock(block.getLocation(),
					Material.GRASS, Material.AIR, 5);
			if (plantBlock != null) {
				// sugar cranes can only grow near water :O
				if (blocks.isAdjacentofBlockType1(plantBlock,
						Material.STATIONARY_WATER)
						|| blocks.isAdjacentofBlockType1(plantBlock,
								Material.WATER)) {
					plantBlock.getRelative(BlockFace.UP).setType(
	...
	public boolean growSingleCane(Block block) {
		if (blocks.isAdjacentofBlockType1(block, Material.STATIONARY_WATER)
				|| blocks.isAdjacentofBlockType1(block, Material.WATER)) {
