...
public class Vegetation extends JavaPlugin
{
	...
	public static PlayerListeners pListener = null;
	public static BlockListeners bListener = null;
    ...
    public void onEnable() 
    {
		...
		pListener = new PlayerListeners(plugin);
		bListener = new BlockListeners(plugin);
		
		pluginName = this.getDescription().getName();
		pluginVersion = this.getDescription().getVersion();
		
		pm.registerEvent(Type.BLOCK_PLACE, bListener, Event.Priority.Normal, plugin );
		pm.registerEvent(Type.PLAYER_INTERACT, pListener, Event.Priority.Normal, plugin );
		pm.registerEvent(Type.PLAYER_QUIT, pListener, Event.Priority.Normal, plugin );
    ...
    {
    	getPlayerList();
    	Integer task = timer.scheduleSyncRepeatingTask(this, new TimerTasks(this), interval, period );
        return task;
    }
	...
	public boolean onCommand(CommandSender sender, Command command, String commandLabel, String[] args) 
    {
    	...
    	{
			sender.sendMessage( "Growing the cacti.." );
		...
		for( X = 0; X < WC; X++ )
		{
			...
			for( Y = 0; Y < PC; Y++ )
			{
				if( world.getPlayers().get(Y).isOnline() )
					Vegetation.playerList.add(world.getPlayers().get(Y));
			}
		}
