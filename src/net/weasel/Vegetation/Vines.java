...
public class Vines {
	...
	public void growVines(Block log) {
		...
		if (lowerLeaves != null) {
			...
			if (r <= lowerLeaves.size()) {
				...
				// let's first try how long the vine can get, we don't want too
				// short vines
	...
	public ArrayList<Block> getLowerLeafBlocks(ArrayList<Block> treeTrunk) {
		...
		if (treeTrunk.size() > 6) {
			...
			// tree is too small to grow vines (only one block between leaves
			// and ground
			return null;
		} else {
			// normal sized tree
		...
		// logOutput("Tree Leaves scanned field of " + blockCountInRange + " " +
		// count + " was: " + LowerLeaves.size() );
